...
- Create a GitHub release whose tag matches the integration version in `manifest.json` (currently `1.2.1`). The release **must** attach the `dist/willow-home-assistant.zip` artifact built by `scripts/build_release.py`, since `hacs.json` uses `zip_release`.
